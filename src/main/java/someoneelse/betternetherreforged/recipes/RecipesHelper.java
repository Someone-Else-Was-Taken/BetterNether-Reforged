...
public class RecipesHelper {
	...
	public static void makeStairsRecipe(Block source, Block stairs) {
		String group = Registry.BLOCK.getKey(stairs).getPath().contains("roof_tile") ? "roof_tile_stairs" : stairs.getSoundType(stairs.getDefaultState()) == SoundType.WOOD ? "nether_wooden_stairs" : "nether_rock_stairs";
		makeSingleRecipe(group, source, stairs, SHAPE_STAIR, 4);
	}

	public static void makeSlabRecipe(Block source, Block slab) {
		String group = Registry.BLOCK.getKey(slab).getPath().contains("roof_tile") ? "roof_tile_slab" : slab.getSoundType(slab.getDefaultState()) == SoundType.WOOD ? "nether_wooden_slab" : "nether_rock_slab";
